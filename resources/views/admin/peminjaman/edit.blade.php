...
@section('content')
    <div class="container">
        ...
        <form action="{{ route('admin.peminjaman.update', $peminjaman->id) }}" method="POST">
            ...
            <div class="mb-3">
                <label>Jumlah</label>
                <input type="number" name="jumlah" class="form-control" min="1" value="{{ $peminjaman->jumlah }}"
                    required>
            </div>

            <div class="mb-3">
                <label>Keperluan</label>
                <textarea name="keperluan" class="form-control" rows="3">{{ $peminjaman->keperluan }}</textarea>
            </div>
